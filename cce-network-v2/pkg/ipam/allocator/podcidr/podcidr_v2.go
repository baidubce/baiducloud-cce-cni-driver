...
type nodeAction struct {
	...
	// reflected in the NetResourceSet CRD and therefore needs to be resynced.
	needsResync bool
}
...
func buildNodeAction(
	...
	// in which the CIDRs are listed in the NetResourceSet CRD.
	...
	// NetResourceSet CRD, we want to resync the CRD to ensure they get added back in.
	for _, podCIDR := range allocatedCIDRs {
...
// updateNode is set to true if the NetResourceSet CRD needs to be updated
// based on the determined node actions
func determineNodeActions(node *v2.NetResourceSet, hasV4Allocators, hasV6Allocators bool, v4PodCIDRs, v6PodCIDRs []*net.IPNet) (v4Action, v6Action nodeAction, err error) {
...
func extractPodCIDRs(node *v2.NetResourceSet) (
...
func (n *NodesPodCIDRManager) allocateNodeV2(node *v2.NetResourceSet) (cn *v2.NetResourceSet, updateSpec, updateStatus bool, err error) {
	...
	log.WithFields(logrus.Fields{
		"v4-pod-cidrs":        logfields.Repr(v4PodCIDRs),
		"v6-pod-cidrs":        logfields.Repr(v6PodCIDRs),
		"v4-changed":          v4Changed,
		"v6-changed":          v6Changed,
		"update-spec":         updateSpec,
		"update-status":       updateStatus,
		"error":               err,
		"postpone-allocation": postponeAllocation,
	}).Debug("Performed node actions")
