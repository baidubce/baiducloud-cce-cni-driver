...
import (
	...
	"github.com/baidubce/baiducloud-cce-cni-driver/cce-network-v2/pkg/versioncheck"
)
...
type ServerCapabilities struct {
	...
	// required to run CCE has been met
	...
	// we don't support HA mode for the cce-operator.
...
const (
	// MinimalVersionConstraint is the minimal version that CCE supports to
	// run kubernetes.
...
func endpointSlicesFallbackDiscovery(client kubernetes.Interface) error {
	...
	// which require CCE to function correctly. By treating failure to
	// discover API groups as fatal, a critial loop can be entered in which
	// CCE cannot start because the API groups can't be discovered.
	//
...
func Update(client kubernetes.Interface, conf k8sconfig.Configuration) error {
	...
	if conf.K8sAPIDiscoveryEnabled() {
		...
		// readiness of regular pods which require CCE to function
		// correctly. By treating failure to discover API groups as
		// fatal, a critical loop can be entered in which CCE cannot
		// start because the API groups can't be discovered and th API
		// groups will only become discoverable once CCE is up.
