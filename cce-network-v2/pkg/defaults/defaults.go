...
const (
	...
	RuntimePath = "/var/run/cce-network-v2"
	...
	// LibraryPath is the default path to the cce libraries directory
	LibraryPath = "/var/lib/cce-network-v2"

	// SockPath is the path to the UNIX domain socket exposing the API to clients locally
	SockPath = RuntimePath + "/cce-network.sock"

	// SockPathEnv is the environment variable to overwrite SockPath
	SockPathEnv = "CCE_SOCK"
	...
	PidFilePath = RuntimePath + "/cce-network-v2.pid"
	...
	DefaultMapRootFallback = "/run/cce/bpffs"
	...
	// which are not part of CCE manged pods.
	EncryptNode = false
	...
	// ClientConnectTimeout is the time the cce-agent client is
	...
	CCEEndpointGCInterval = 30 * time.Second
	...
	// NetResourceSet.Spec.IPAM.PreAllocate if no value is set
	IPAMPreAllocation = 1

	// ENIFirstInterfaceIndex is the default value for
	// NetResourceSet.Spec.ENI.FirstInterfaceIndex if no value is set.
	ENIFirstInterfaceIndex = 1

	// UseENIPrimaryAddress is the default value for
	// NetResourceSet.Spec.ENI.UsePrimaryAddress if no value is set.
	...
	// CloudAPIQPSLimit is the default QPS limit when rate limiting access to external APIs
	CloudAPIQPSLimit = 20.0

	// IPAMPodCIDRAllocationThreshold is the default value for
	// NetResourceSet.Spec.IPAM.PodCIDRAllocationThreshold if no value is set
	...
	// NetResourceSet.Spec.IPAM.PodCIDRReleaseThreshold if no value is set
	...
	AutoCreateNetResourceSetResource = true
	...
	// which denotes the value of CCE's K8s service cache size.
	...
	// RestoreV4Addr is used as match for cce_host v4 address
	RestoreV4Addr = "cce.v4.internal.raw "

	// RestoreV6Addr is used as match for cce_host v6 (router) address
	RestoreV6Addr = "cce.v6.internal.raw "
	...
	// EnableICMPRules enables ICMP-based rule support for CCE Network Policies.
