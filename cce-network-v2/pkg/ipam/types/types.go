...
type AllocationIP struct {
	...
	// SubnetID is the subnet ID of the IP. This field is set if the IP is use in cross subnet mode
	//
	// +optional
	SubnetID string `json:"subnetID,omitempty"`

	// IsPrimary is set to true if the IP is the primary IP of the eni
...
//
// This structure is embedded into v2.NetResourceSet
type IPAMSpec struct {
	...
	// addresses available immediately without requiring cce-operator to
	// get involved.
	//
	// +kubebuilder:validation:Minimum=0
	...
	// cce-operator will allocate a new pod CIDR to this node.
	// This value effectively defines the buffer of IP addresses available
	// immediately without requiring cce-operator to get involved.
	//
	// +kubebuilder:validation:Minimum=0
	...
	// cce-agent will attempt to release currently unused pod CIDRs.
	//
	// +kubebuilder:validation:Minimum=0
...
type IPAMStatus struct {
	...
	// CrossSubnetUsed lists all IPs out of Spec.IPAM.Pool which have been allocated
	//
	// +optional
	CrossSubnetUsed AllocationMap `json:"crossSubnetUsed,omitempty"`
	...
	// VPCRouteCIDRs lists the status of each VPC route CIDR allocated to this node.
	VPCRouteCIDRs VPCRouteStatuMap `json:"vpc-route-cidrs,omitempty"`
...
type VPCRouteStatus string

const (
	VPCRouteStatusReleased VPCRouteStatus = "released"
	VPCRouteStatusInUse    VPCRouteStatus = "in-use"
)
...
// OperatorStatus is the status used by cce-operator to report
// errors in case the allocation CIDR failed.
type OperatorStatus struct {
	// Error is the error message set by cce-operator.
	//
	// +optional
