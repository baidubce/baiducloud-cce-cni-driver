...
type prometheusMetrics struct {
	...
	NetResourceSets       *prometheus.GaugeVec
...
func NewPrometheusMetrics(namespace string, registry *prometheus.Registry) *prometheusMetrics {
	...
	m.NetResourceSets = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Namespace: namespace,
		Subsystem: ipamSubsystem,
		Name:      "nodes",
	...
	registry.MustRegister(m.NetResourceSets)
...
func (p *prometheusMetrics) SetNetResourceSets(label string, nodes int) {
	p.NetResourceSets.WithLabelValues(label).Set(float64(nodes))
}
