...
func K8sErrorHandler(e error) {
	...
	errstr := e.Error()
	switch {
	// This can occur when cce comes up before the k8s API server, and keeps
	// trying to connect.
	...
	// the k8s cluster, and might occur when upgrading from versions of cce
	// that used ThirdPartyResource to define CCENetworkPolicy.
	case strings.Contains(errstr, "Failed to list *v2.CCENetworkPolicy: the server could not find the requested resource"):
		if k8sErrorUpdateCheckUnmuteTime(errstr, now) {
			log.WithError(e).Error("No CCE Network Policy CRD defined in the cluster, please set `--skip-crd-creation=false` to avoid seeing this error.")
		}
	...
	case strings.Contains(errstr, "Unable to decode an event from the watch stream: unable to decode watch event"),
		strings.Contains(errstr, "Failed to list *v1.CCENetworkPolicy: only encoded map or array can be decoded into a struct"),
		strings.Contains(errstr, "Failed to list *v2.CCENetworkPolicy: only encoded map or array can be decoded into a struct"),
		strings.Contains(errstr, "Failed to list *v2.CCENetworkPolicy: v2.CCENetworkPolicyList:"):
		if k8sErrorUpdateCheckUnmuteTime(errstr, now) {
