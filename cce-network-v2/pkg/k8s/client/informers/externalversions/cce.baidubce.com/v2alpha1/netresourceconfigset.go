...
// NetResourceConfigSetInformer provides access to a shared informer and lister for
...
type netResourceConfigSetInformer struct {
	factory          internalinterfaces.SharedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

// NewNetResourceConfigSetInformer constructs a new informer for NetResourceConfigSet type.
...
func NewNetResourceConfigSetInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredNetResourceConfigSetInformer(client, resyncPeriod, indexers, nil)
}

// NewFilteredNetResourceConfigSetInformer constructs a new informer for NetResourceConfigSet type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredNetResourceConfigSetInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		...
		&ccebaidubcecomv2alpha1.NetResourceConfigSet{},
		resyncPeriod,
		indexers,
	)
}

func (f *netResourceConfigSetInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredNetResourceConfigSetInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *netResourceConfigSetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&ccebaidubcecomv2alpha1.NetResourceConfigSet{}, f.defaultInformer)
}

func (f *netResourceConfigSetInformer) Lister() v2alpha1.NetResourceConfigSetLister {
	return v2alpha1.NewNetResourceConfigSetLister(f.Informer().GetIndexer())
}
