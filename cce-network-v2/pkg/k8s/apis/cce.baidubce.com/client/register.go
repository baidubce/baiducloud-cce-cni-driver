...
func CreateCustomResourceDefinitions(clientset apiextensionsclient.Interface) error {
	...
	for _, r := range synced.AllCRDResourceNames() {
		...
		if !ok {
			log.Fatalf("Unknown resource %s. Please update pkg/k8s/apis/cce.baidubce.com/client to understand this type.", r)
		}
		g.Go(func() error {
			return fn(clientset)
		})
...
var (
	//go:embed crds/v2/cceendpoints.yaml
	crdsCCEendpoints []byte

	//go:embed crds/v2/netresourcesets.yaml
	crdsNetResourceSets []byte
)
...
func GetPregeneratedCRD(crdName string) apiextensionsv1.CustomResourceDefinition {
	...
	switch crdName {
	case CEPCRDName:
		crdBytes = crdsCCEendpoints
	case CNCRDName:
		crdBytes = crdsNetResourceSets
	default:
		scopedLog.Fatal("Pregenerated CRD does not exist")
	}

	cceCRD := apiextensionsv1.CustomResourceDefinition{}
	err = yaml.Unmarshal(crdBytes, &cceCRD)
	if err != nil {
...
// createCEPCRD creates and updates the CCEEndpoint CRD. It should be called
// on agent startup but is idempotent and safe to call again.
func createCEPCRD(clientset apiextensionsclient.Interface) error {
	cceCRD := GetPregeneratedCRD(CEPCRDName)

	return createUpdateCRD(
		clientset,
		CEPCRDName,
		constructV1CRD(k8sconstv2.CEPName, cceCRD),
		newDefaultPoller(),
	)
}

// createNetResourceSetCRD creates and updates the NetResourceSet CRD. It should be called on
// agent startup but is idempotent and safe to call again.
func createNetResourceSetCRD(clientset apiextensionsclient.Interface) error {
	cceCRD := GetPregeneratedCRD(CNCRDName)

	return createUpdateCRD(
		clientset,
		CNCRDName,
		constructV1CRD(k8sconstv2.NRSName, cceCRD),
		newDefaultPoller(),
	)
}
