...
// eNILister implements the ENILister interface.
...
func NewENILister(indexer cache.Indexer) ENILister {
	return &eNILister{indexer: indexer}
}

// List lists all ENIs in the indexer.
func (s *eNILister) List(selector labels.Selector) (ret []*v2.ENI, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.ENI))
	})
	return ret, err
}

// Get retrieves the ENI from the index for a given name.
func (s *eNILister) Get(name string) (*v2.ENI, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("eni"), name)
	}
	return obj.(*v2.ENI), nil
}
