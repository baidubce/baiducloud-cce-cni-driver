...
func (in *EndpointIdentifiers) DeepEqual(other *EndpointIdentifiers) bool {
	...
	if in.Cnidriver != other.Cnidriver {
		return false
	}
...
func (in *EndpointProbeResponse) DeepEqual(other *EndpointProbeResponse) bool {
	...
	if ((in.FeatureGates != nil) && (other.FeatureGates != nil)) || ((in.FeatureGates == nil) != (other.FeatureGates == nil)) {
		in, other := &in.FeatureGates, &other.FeatureGates
		if other == nil {
			return false
		}
		...
		if len(*in) != len(*other) {
			...
			for i, inElement := range *in {
				if !reflect.DeepEqual(inElement, (*other)[i]) {
					return false
				}
			}
		}
