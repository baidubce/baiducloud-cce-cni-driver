...
var vifFeatures2IPType = map[string]rpc.IPType{
	"elastic_rdma": rpc.IPType_ERIENIMultiIPType,  //eri
	"rdma_roce":    rpc.IPType_RoceENIMultiIPType, //roce
}

var MaxRetries = 5
var InitialDelay = time.Second
var Factor = 2.0
...
func (ipam *roceIPAM) AllocIP(ctx context.Context, k8sArgs *cni.K8SArgs, endpoint string, masterMac string, vifFeatures string) (*rpc.AllocateIPReply, error) {
	...
	ipType, ok := vifFeatures2IPType[strings.TrimSpace(vifFeatures)]
	if !ok {
		log.Errorf(ctx, "unexpected vif features %v", vifFeatures)
		return nil, fmt.Errorf("unexpected vif features %v", vifFeatures)
	}
	c := ipam.rpc.NewCNIBackendClient(conn)

	return ipam.RetryWithBackoff(ctx, c, k8sArgs, masterMac, ipType, MaxRetries, InitialDelay, Factor)
}
...
func (ipam *roceIPAM) RetryWithBackoff(ctx context.Context, rpcClient rpc.CNIBackendClient,
	...
	initialDelay time.Duration, factor float64) (*rpc.AllocateIPReply, error) {
	delay := initialDelay
	...
	for i := 0; i < maxRetries; i++ {
		...
		log.Infof(ctx, "retry %d failed: %v\n", i+1, err)
		time.Sleep(delay)
