...
import (
	...
	utiliptables "github.com/baidubce/baiducloud-cce-cni-driver/pkg/util/iptables"
...
type MasqConfig struct {
	...
	MasqOutBound       bool     `json:"masqOutBound"`
	MasqOutBoundIPv6   bool     `json:"masqOutBoundIPv6"`
...
func (m *MasqDaemon) Run() {
	...
	for {
		func() {
			...
			ctx := log.NewContext()
			// resync config
			if err := m.osSyncConfig(ctx); err != nil {
				log.Errorf(ctx, "error syncing configuration: %v", err)
				return
			}
			// resync rules
...
func (m *MasqDaemon) osSyncConfig(ctx context.Context) error {
	// the fs.FileSystem interface allows us to mock the fs from tests
	// fs.DefaultFS implements fs.FileSystem using os.Stat and io/ioutil.ReadFile
	var fs fs.FileSystem = fs.DefaultFS{}
	return m.syncConfig(ctx, fs)
}
...
func (m *MasqDaemon) syncConfig(ctx context.Context, fs fs.FileSystem) error {
	var err error
	c := NewMasqConfig(*noMasqueradeAllReservedRangesFlag)
	defer func() {
		if err == nil {
			json, _ := utiljson.Marshal(c)
			log.Infof(ctx, "using config: %s", string(json))
		}
	}()
	...
	if _, err = fs.Stat(configPath); os.IsNotExist(err) {
		...
		log.Infof(ctx, "no config file found at %q, using default values", configPath)
		return nil
	}
	log.Infof(ctx, "config file found at %q", configPath)
...
func (c *MasqConfig) validate() error {
	...
	var cidrs []string
	cidrs = append(cidrs, c.NonMasqueradeCIDRs...)
	cidrs = append(cidrs, c.MasqueradeCIDRs...)
...
const masqCIDRRuleComment = `-m comment --comment "ip-masq-agent: specific outbound traffic is subject to MASQUERADE"`

func writeMasqCIDRRule(lines *bytes.Buffer, cidr string) {
	writeRule(lines, utiliptables.Append, masqChain, masqCIDRRuleComment, "-d", cidr, "-j", "MASQUERADE")
}
