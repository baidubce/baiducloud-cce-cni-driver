...
type crossVpcEniIPAM struct {
	gRPCClient grpcwrapper.Interface
	rpc        rpcwrapper.Interface
}

func NewCrossVpcEniIPAM(grpc grpcwrapper.Interface, rpc rpcwrapper.Interface) *crossVpcEniIPAM {
	return &crossVpcEniIPAM{
		gRPCClient: grpc,
		rpc:        rpc,
	}
}

func (ipam *crossVpcEniIPAM) CreateEni(ctx context.Context, k8sArgs *cni.K8SArgs, endpoint string) (*rpc.AllocateIPReply, error) {
...
func (ipam *crossVpcEniIPAM) DeleteEni(ctx context.Context, k8sArgs *cni.K8SArgs, endpoint string) (*rpc.ReleaseIPReply, error) {
	ctx, cancel := context.WithTimeout(ctx, rpcTimeout)
	defer cancel()
