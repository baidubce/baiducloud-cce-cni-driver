...
var (
	mutatingWebhookConfigurationName   = "cce-eni-ipam-mutating-webhook-configuration"
	validatingWebhookConfigurationName = "cce-eni-ipam-validating-webhook-configuration"
)

func configurationEnsure(kubeClient clientset.Interface, handlers map[string]admission.Handler, caBundle []byte) error {
	err := ensureMutatingWebhook(kubeClient, handlers, caBundle)
	if err != nil {
		return err
	}

	return ensureValidatingWebhook(kubeClient, handlers, caBundle)
}

func ensureMutatingWebhook(kubeClient clientset.Interface, handlers map[string]admission.Handler, caBundle []byte) error {
...
func ensureValidatingWebhook(kubeClient clientset.Interface, handlers map[string]admission.Handler, caBundle []byte) error {
